...
const initPage = function() {
  ...
  if (typeof initDropzone === 'function') {
      initDropzone('dropzone', 'arquivo_docx', '.docx', window.processarArquivo);
  }

  const btnAdd = document.getElementById('btnAddCampo');
  if(btnAdd) {
      btnAdd.onclick = () => window.adicionarCampo();
  }

  const btnRemove = document.getElementById('btnRemoveFile');
  if(btnRemove) {
      btnRemove.onclick = () => window.removerArquivo();
  }
};
...
window.extrairPlaceholders = async function (file) {
  try {
...
// ── Campos builder ─────────────────────────────────────────────────────────
window.adicionarCampo = function (name = '', label = '', tipo = 'text', format = '') {
  const id = ++campoCount;
  ...
  row.innerHTML = `
    ...
    <select class="campo-tipo">
      ${Object.entries(tipos).map(([v, t]) => `<option value="${t}" ${v===tipos?'selected':''}>${t}</option>`).join('')}
    </select>
    <select class="campo-format">
      ${Object.entries(formatos).map(([v, t]) => `<option value="${v}" ${v===format?'selected':''}>${t}</option>`).join('')}
    </select>
...
window.salvarTemplate = async function () {
  ...
  document.querySelectorAll('.campo-row').forEach(row => {
    ...
    const f = row.querySelector('.campo-format').value;

    if (!n) { erros.push('Preencha o nome de todos os campos.'); return; }

    campos.push({ name: n, label: l || n, type: t, format: f });
  });
  ...
  try {
    const r = await fetch('actions/upload_template.php', { method: 'POST', body: fd });

    const textoDeResposta = await r.text();
    console.log("RESPOSTA DO SERVIDOR:", textoDeResposta);

    try {
        const d = JSON.parse(textoDeResposta);
        if (d.sucesso) {
        ...
        console.error("Erro ao converter JSON. O servidor mandou isso:", textoDeResposta);
        toast('error', 'Erro no servidor. Veja o Console (F12)');
    }
    // const r = await fetch('/generate_word/actions/upload_template.php', { method: 'POST', body: fd });
...
window.excluirTemplate = async function (id, nome) {
