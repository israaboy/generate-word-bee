...
$funcionarios = $db->query("
    ...
    WHERE cod_empresa IN (26, 523, 68, 364, 365, 157, 234)
    ORDER BY nome_funcionario
")->fetchAll();

layout_head('Preencher Formulário');
?>
...
<div id="stepResultado" style="display:none;">
  <div class="card" style="text-align:center; padding:3rem 2rem;">
    ...
    <div class="flex gap-1" style="justify-content:center; flex-wrap:wrap;">
      ...
      <a href="<?= BASE_URL ?>/admin.php" class="btn btn-outline">Ver no Admin</a>
    </div>
  </div>
</div>
...
function carregarCampos(idTipo) {
  fetch('<?= BASE_URL ?>/actions/get_campos.php?id_tipo=' + idTipo)
...
function gerarDocumento() {
  ...
  fetch('<?= BASE_URL ?>/actions/gerar_documento.php', { method: 'POST', body: fd })
    .then(function(r) { return r.json(); })
    .then(function(d) {
      if (d.sucesso) {
